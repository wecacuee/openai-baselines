#PBS -N fwrl-her-train-loss-term-weights  # Any name to identify your job
#PBS -j oe                   # Join error and output files for convinience
#PBS -l walltime=1000:00:00     # Keep walltime big enough to finish the job
#PBS -l nodes=eyeball:ppn=1:gpus=1 # nodes requested: Processor per node: gpus requested
...
. setup.sh
python <<EOF
from baselines.her.experiment.train_many import train_loss_term_weights
train_loss_term_weights(num_cpu = 6)
EOF
