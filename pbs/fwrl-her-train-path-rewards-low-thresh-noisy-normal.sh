#PBS -N fwrl-her-train-path-rewards-low-thresh-noisy-normal  # Any name to identify your job
...
. setup.sh
python <<EOF
from baselines.her.experiment.train_many import (exp_conf_path_reward_low_thresh_noisy_normal ,
                                                 run_one_experiment)
exp_conf, common_kwargs = exp_conf_path_reward_low_thresh_noisy_normal(num_cpu = 6, gitrev = "3a91283")
...
# ## >>> print("\n".join(map(str, enumerate(exp_conf_path_reward_low_thresh_noisy_normal()[0].items()))))
